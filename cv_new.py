...
for i in range(len(training_files_list)):
    ...
    X_train_raw, Y_train_raw, X_test_raw, Y_test_raw, chans, kernels, samples, names, sample_rate = util.get_bci_2a(training_files, test_files, bandpass = bandpass,tmin = tmin, tmax = tmax,mode = "gdf",amp_mag= amplitude_magnification, baseline=baseline)
    ...
    for fold_step, (train_index, val_index) in enumerate(train_val_split):
        ...
        X_train, X_test, X_validate, Y_train, Y_validate, Y_test, freq_bins_centers, time_window_centers = util.prepare_data(X_train_raw,  X_test_raw, Y_train_raw, Y_test_raw, sample_rate, segment_len, sample_overlap, boundary, padding, input_format, chans, samples, kernels, n_freqs, True, train_index,val_index)
        ...
        _, acc = util.predict_and_visualise(X_test, Y_test, model, fittedModelHistory, names, i, sum_accuracies, False, fold_step)

        fold_accuracies.append(acc)
    
    fold_accuracy = (sum(fold_accuracies)) / 3
